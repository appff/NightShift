...
- **Logic**: This protocol acts as an automated, evidence-based code reviewer. It mechanically checks the execution log against a set of persona-aware rules:
    - **Is it a coding task?** If so, `tests_passed` is mandatory. It looks for evidence like `pytest` results (`OK`, `passed`). For non-coding tasks, this check is skipped.
    - **Is the persona's core duty met?**
        - For a `technical-writer`, it checks if a "README" or "guide" was actually created.
        - For a `deep-research-agent`, it verifies that sources ("http", "source:") are cited in the output.
        - For a `refactoring-expert`, it looks for keywords like "refactor" or "simplify" in the logs.
    - **Was there evidence of work?** It checks if files were changed or if the execution log is substantial.
- **Signal**:
